...
void drw_rect(Drw *drw, int x, int y, unsigned w, unsigned h, int filled, int invert) {
...
int drw_text(Drw *drw, int x, int y, unsigned w, unsigned h, unsigned lpad, const char *text, int invert) {
	char		buf[1024];
	int			ty;
	unsigned	ew = 0;
	XftDraw	*d  = NULL;
	...
	while (1) {
		...
		if (utf8strlen) {
			...
			for (len = MIN(utf8strlen, sizeof(buf) - 1); len && ew > w; len--) drw_font_getexts(usedfont, utf8str, len, &ew, NULL);
		...
		if (!*text) break;
			...
			if (!drw->fonts->pattern) die("the first font in the cache must be loaded from a font string.");
...
void drw_font_getexts(Fnt *font, const char *text, unsigned len, unsigned *w, unsigned *h) {
	XGlyphInfo ext;
	if (!font || !text) return;
