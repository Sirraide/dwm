...
_Static_assert(LENGTH(tags) <= 31, "Too many tags!");
...
void applyrules(Client *c) {
	...
	for (unsigned i = 0; i < LENGTH(rules); i++) {
...
int applysizehints(Client *c, int *x, int *y, int *w, int *h, int interact) {
	...
	*h = MAX(1, *h);
	...
	/// always true by default but may be changed in config
...
void buttonpress(XEvent *e) {
	...
	if (ev->window == selmon->barwin) {
		...
		else if (ev->x > selmon->ww - (int) TEXTW(stext)) click = ClkStatusText;
...
void drawbar(Monitor *m) {
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (occ & 1 << i) drw_rect(drw, x + boxs, boxs, boxw, boxw,
...
int gettextprop(Window w, Atom atom, char *text, unsigned size) {
	char		 **list = NULL;
	int			  n;
	XTextProperty name;
...
void grabbuttons(Client *c, int focused) {
	updatenumlockmask();
	{
		...
		if (!focused) XGrabButton(dpy, AnyButton, AnyModifier, c->win, False,
			BUTTONMASK, GrabModeSync, GrabModeSync, None, None);
		for (i = 0; i < LENGTH(buttons); i++)
			if (buttons[i].click == ClkClientWin)
				for (j = 0; j < LENGTH(modifiers); j++) XGrabButton(dpy, buttons[i].button,
...
static int
	...
	while (n--)
		if (unique[n].x_org == info->x_org && unique[n].y_org == info->y_org
			&& unique[n].width == info->width && unique[n].height == info->height) return 0;
	return 1;
}
#endif /* XINERAMA */
...
void monocle(Monitor *m) {
	...
	if (n > 0) snprintf(m->ltsymbol, sizeof m->ltsymbol, "[%d]", n);
			...
			case MotionNotify:
				...
				if (c->mon->wx + nw >= selmon->wx
					...
					if (!c->isfloating
						...
						&& (abs(nw - c->w) > snap || abs(nh - c->h) > snap)) togglefloating(NULL);
...
void togglefloating(const Arg *arg) {
	if (!selmon->sel) return;
	if (selmon->sel->isfullscreen) return; /* no support for fullscreen windows */
...
void updatetitle(Client *c) {
	...
	if (c->name[0] == '\0') strcpy(c->name, broken); /* hack to mark broken clients */
}
